<?php
namespace app\models;

use Yii;

/**
 * This is the model class for table "pemesanan".
 *
 * @property integer $id
 * @property string  $tgl
 * @property integer $id_supplier
 * @property string  $subtotal
 * @property string  $ppn
 * @property string  $disc
 * @property string  $total
 * @property integer $insert_user
 * @property string  $insert_date
 * @property integer $update_user
 * @property string  $update_date
 * @property string  $status
 */
class Pemesanan extends \yii\db\ActiveRecord
{

    /**
     * @inheritdoc
     */
    public static function tableName()
    {
        return 'pemesanan';
    }

    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['tgl', 'id_supplier'], 'required'],
            [['tgl', 'insert_date', 'update_date'], 'safe'],
            [['id_supplier', 'insert_user', 'update_user'], 'integer'],
            [['subtotal', 'ppn', 'disc', 'total'], 'number'],
            [['status'], 'string'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function attributeLabels()
    {
        return [
            'id'          => 'ID',
            'tgl'         => 'Tgl. Pemesanan',
            'id_supplier' => 'Nama Supplier',
            'subtotal'    => 'Subtotal',
            'ppn'         => 'PPN',
            'disc'        => 'Discount',
            'total'       => 'Total',
            'insert_user' => 'Insert User',
            'insert_date' => 'Insert Date',
            'update_user' => 'Update User',
            'update_date' => 'Update Date',
            'status'      => 'status',
        ];
    }

    /**
     * @inheritdoc
     * @return PemesananQuery the active query used by this AR class.
     */
    public static function find()
    {
        return new PemesananQuery(get_called_class());
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getSupplier()
    {
        return $this->hasOne(Supplier::className(), ['id' => 'id_supplier']);
    }
}
